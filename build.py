...
# oxide for generative AI and agents, pine for statistics and analysis, ink otherwise
ACCENT = {
    "featured": "sec-ink",
    "applied-ai": "sec-oxide",
    "genai": "sec-oxide",
    "running": "sec-pine",
    "language": "sec-pine",
    "consumer": "sec-pine",
...
for i, s in enumerate(data["sections"], 1):
    num = f"{i:02d}"
    cls = ACCENT.get(s["id"], "sec-ink")
    lede = f'<p class="section-lede">{e(s["lede"])}</p>' if s.get("lede") else ""
    sections.append(
        f'<section class="{cls}"><h2 id="{s["id"]}">'
        f'<span class="sec-num">{num}</span>{e(s["title"])}</h2>{lede}{table(s["projects"])}</section>')
    toc.append(
        f'<li class="{cls}"><a href="#{s["id"]}"><span><span class="n">{num}</span>{e(s["title"])}</span>'
        f'<span class="count">{len(s["projects"])}</span></a></li>')
...
<header class="masthead">
  <a class="brand" href="{PAGES}"><b>Jeremy Lee</b> / portfolio</a>
  <nav aria-label="Page">
    <a href="#contents">Projects</a>
...
<p class="hero-note">All {n} projects below are live, with code on <a href="https://github.com/lyhjeremy">my GitHub profile</a>. Design and copy follow <a href="https://github.com/lyhjeremy/lyhjeremy/blob/main/DESIGN_STANDARDS.md">these standards</a>; this page's source is <a href="{GH}lyhjeremy.github.io">lyhjeremy.github.io</a>.</p>
<pre class="cmd">npx lyhjeremy</pre>
<p class="pre-cap">Run that in a terminal for the interactive version of this page.</p>

<nav class="toc" id="contents" aria-label="Sections">
<h2>Contents</h2>
<ol>
{"".join(toc)}
</ol>
...
<h2 id="skills"><span class="sec-num">08</span>What I work with</h2>
<dl class="skills">
...
<section class="sec-ink">
<h2 id="experience"><span class="sec-num">09</span>Experience and education</h2>
<ul>
...
<p>Outside the terminal: 14 marathons, Berlin personal best of 2:48. WSET Level 3 in wine.</p>
</section>

<section class="sec-ink">
<h2 id="contact"><span class="sec-num">10</span>Contact</h2>
